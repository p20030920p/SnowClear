...
def main() -> int:
    ...
    snow[det] = True
    # The node publishes the *ROI* cloud as its de-snowed output, not the whole frame:
    # reproduce that, or the display shows 200 k points instead of the ~54 k the
    # shipped layout is designed around.
    ...
    raw, desnowed, removed = pts, pts[roi & ~snow], pts[snow]
    print(f"raw {raw.shape[0]}  de-snowed {desnowed.shape[0]}  removed {removed.shape[0]}")

    rclpy.init()
    node = Node("rviz_feed")
    pubs = (
        node.create_publisher(PointCloud2, "/snowclear/input/points", qos_profile_sensor_data),
        node.create_publisher(PointCloud2, "/snowclear/output/points", qos_profile_sensor_data),
        node.create_publisher(PointCloud2, "/snowclear/output/snow_points", qos_profile_sensor_data),
    )
    period = 1.0 / a.rate if a.rate > 0 else 0.0
    for i in range(a.cycles):
        stamp = node.get_clock().now().to_msg()
        for pub, cloud in zip(pubs, (raw, desnowed, removed)):
