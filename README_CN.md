...
| **速度** —— 仅用 CPU，不用 GPU | **≈ 10 ms**/帧 |
| **测试平台** | 轻薄本：华为 MateBook 14（2022 款）· Intel Core i5-1240P（12 核 16 线程，28 W）· Ubuntu 24.04 · Release 构建 · `OMP_NUM_THREADS=2`，即 16 线程里只用 2 个 |
| **同一批 101 帧对比 CRFOR（Wang et al., RA-L 2023）** | F1 96.90 vs 96.41，速度快 **≈ 1 900 倍**（9 ms vs 17 132 ms 每帧，同一台机器） |
| **是否需要训练** | 不需要 |
