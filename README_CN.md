...
![每帧六面板：原始点云、去雪后点云与检测结果，上排为 SnowClear、下排为真值](docs/figures/detect_scene35.gif)

*一个场景、连续 21 帧、固定视角 —— 上排 **SnowClear**、下排 **ground truth**，每排依次为
**原始点云 → 去雪后 → 结果**。绿色为检出且被标注（TP），红色为检出但无标注（FP），蓝色为被标注却
漏检（FN）；顶部给出该帧 ROI 内的精确率 / 召回率 / F1。两排对着看才是重点：衡量一个去雪方法，问题不是
"图看着干不干净"，而是上排离下排有多远。*
...
16 个场景 / 1 620 帧上按场景取的宏平均。下文每张图都由本仓库内的工具生成，命令见
[`docs/figures/README.md`](docs/figures/README.md)。

![SnowClear 与四个非学习基线的精确率 / 召回率 / F1（报告集宏平均）](docs/figures/fig3_comparison_zh.png)

*与非学习基线的对比 —— 四个滤波器都编译在核心里，共用完全相同的 ROI 门控、索引映射与评测路径，
因此这一比较隔离出的是判定规则本身。*
