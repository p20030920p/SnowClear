...
*帧 `042126`，所有面板共用同一真值；右下记分板重复各方法 ROI 内的 P / R / F1。CRFOR
（Wang et al., RA-L 2023）按其发布设置与自带预处理运行；其余四个滤波器与 SnowClear 共用我们的流水线。*
...
*发布配置的真值预算：每个标注点计入第一个拒绝它的阶段。绿色段即召回上限 —— 报告集上为 89.90 %，
实测召回 89.98 %；场景 16 上 ROI 门控占 40.5 %、强度上限占 14.6 %。*
