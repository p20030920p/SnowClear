...
*SnowClear 的 F1 是最好的滤波器的 **2.4 倍**，而且差距不是一点点：密度类滤波器的精确率之所以好看，是
因为它们几乎什么都不返回——召回不足 4 %。*
...
*两种失效方式方向相反、且都是致命的：DROR / DSOR / ROR 根本没碰被标注的雪（蓝色），SOR 则把路面一起
削掉（红色）。只有 SnowClear 的面板与真值吻合。*

![同一帧上 SnowClear 与 SOR 的鸟瞰对比](docs/figures/fig11_comparison_zh.png)

*鸟瞰视角下，被剔除的雪是一层覆盖在路面上的薄膜，而不是一团云——这恰恰是 LiDAR 眼里的雪的真实形态。*

### 可移植性

![发布常量与无标注自标定的对比：参考安装高度与抬高 0.9 m](docs/figures/fig7_cross_sensor_zh.png)

*抬高 0.9 m 让发布常量丢掉 **24.25 pp** F1，而自标定只差 0.06 pp。崩掉的是召回（89.97 → 53.42 %），
精确率几乎不动：这些绝对常量不再覆盖世界中的同一片区域。*

### 召回去了哪里

![召回去了哪里：逐场景真值预算](docs/figures/fig8_gt_ceiling_zh.png)

*89.90 % 的真值可达、实测召回 89.98 %，因此剩下的损失来自门控与强度上限，**不是**判定规则。*
...
*101 帧的 F1 稳定在 98.0，而且剔除点数与标注点数基本相当——这说明它没有用召回换精确。三处回落都出现在
雪层很薄的帧上，几个点就能让百分比动一下。*

### 为什么用弱回波判据，以及它永远够不到什么

![弱回波判据的实测，以及逐场景高于上限的标注占比](docs/figures/fig14_intensity_zh.png)

*场景 35 上 99.6 % 的被标注回波强度恰好为 0，而其余点里 98 % 更亮——这就是弱回波判据在这里成立的原因。
但这个上限并不均匀：它挡掉场景 35 的 0.19 % 标注，却挡掉场景 16 的 **27.20 %**，这正是场景 16 召回受限
的原因。*
...
*检测与标注贴合得很好；剩下的只是 ROI 内沿边缘的几个簇——那里正是门控与高度项相互拉扯的地方。*

![场景 16 的召回受限帧：大部分漏检雪点在 ROI 之外](docs/figures/fig10_qualitative_hard_zh.png)

*另一端（召回 44 %）：漏检大多落在 ROI **之外**，属于构造上不可达，而不是判定失误。*
...
*两张图共用一条图注，因为它们是同一个视角：该帧 208 504 点中剔除 6 957 点，呈覆盖在路面上的一层薄膜，
而不是成团的簇。*
...
*判定门的闭式结果：上限正好落在两个强度尖峰之间的空档里，因此在它的整个取值范围内挪动阈值几乎不改变结果。*

![阈值族 T(r, I)、α(r) 权重与逐帧基阈值](docs/figures/fig9_threshold_curve_zh.png)

*`T(r, I)`、塑造它的 `α(r)` 权重，以及在 69.7 % 的帧上被钳在下限的基阈值。*
