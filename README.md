...
*Against non-learned baselines, on the same frames and the same pipeline. SnowClear's F1 is **2.4×**
the best filter's, and the ordering is not close: the density filters are precise only because they
return almost nothing — under 4 % recall.*
...
| *One frame, five detectors, one camera. The failure modes are opposite and both fatal: DROR / DSOR / ROR leave the annotated snow in place (blue), SOR takes the road surface with it (red).* | *The same frame against SOR in bird's-eye view — the classic before/after, where the removed layer is visible as a thin film over the road.* |
...
| *Portability, measured: a +0.9 m mount costs the released constants **24.25 pp** of F1 and the label-free self-calibration 0.06 pp. What breaks is recall — 89.97 → 53.42 % — while precision holds.* | *Where the recall goes: 89.90 % of ground truth stays reachable and the measured recall is 89.98 %, so the remaining loss is the gate and the ceiling, **not** the decision rule.* |
...
*Every frame of the scene above. Quality is flat — F1 98.0 over 101 frames — and the detector removes
about as many points as are annotated, which is the sign that it is not trading precision for recall.
The three dips are frames with a thin snow layer, where a handful of points moves a percentage.*

![The weak-return test, and the share of annotated returns it can never reach, per scene](docs/figures/fig14_intensity.png)

*Why a weak-return test is the right shape here: on scene 35, 99.6 % of annotated returns are exactly
`I = 0` while 98 % of everything else is brighter. The ceiling that follows is not uniform though —
...
| *Reference frame from scene 35: the detection tracks the annotation closely, and what is left over sits in a few clusters just inside the ROI.* | *The other end of the range (scene 16, recall 44 %): the misses are mostly annotated points **outside** the ROI, unreachable by construction rather than by decision.* |
| ![Before and after at 3D perspective](docs/figures/fig0_before.png) | ![The same crop after removal](docs/figures/fig0_after.png) |
| *Before: the full scene and a 5.2 m zoom, with the points SnowClear removes in red.* | *After: the same crop cleaned. 6 957 of 208 504 points removed here — a thin film, not a cloud.* |
...
| *The same frame in 3D: the snow sits **above** the ground surface, which is why the height term helps at the margin and why geometry alone cannot carry the decision.* | *The before/after pair as one card, for slides or print.* |
...
*Per scene, released configuration. Two scenes fall out of the reported band and they are the same two
that carry the largest unreachable share in the figure above — the weakness is systematic, not noise.*

![Single-switch ablation: macro-F1 delta per module on the 4-scene subset](docs/figures/fig4_ablation.png)

*One switch flipped away from the released configuration per run. Every module that ships disabled
costs F1 when enabled, and the grid-search optimiser buys +0.0012 pp while tripling the frame time —
which is why it returns the defaults.*
...
is worth a paired +1.53 ms; the elevation fast path's own saving is below what these timers resolve.*
...
| *The gate in closed form. The ceiling lands in the empty gap between the two intensity spikes, which is why moving the threshold inside its whole range changes almost nothing.* | *The threshold family `T(r, I)`, the `α(r)` weight that shapes it, and the base threshold that is pinned at its floor in 69.7 % of frames.* |
